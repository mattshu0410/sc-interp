"""Per-condition 2D PCA scatter using `torchdr.PCA`."""
...
DEFAULT_OUT_ROOT = REPO_ROOT / "predictions" / "repeval" / "vis" / "pca"
...
def main(argv: list[str] | None = None) -> None:
    p = argparse.ArgumentParser(prog="python -m scripts.repeval.vis.pca")
    ...
    def fit_fn(x: np.ndarray) -> np.ndarray:
        model = PCA(n_components=2, device=args.device)

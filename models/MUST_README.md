...
class GeneVocab:
    def __init__(
        ...
        if isinstance(gene_list_or_vocab, GeneVocab):
            ...
            for tok in gene_list_or_vocab.get_itos():
                self.append_token(tok)
        elif isinstance(gene_list_or_vocab, list):
            self._build_vocab_from_iterator(
                gene_list_or_vocab,
                specials=specials,
            ...
            raise ValueError(
                "gene_list_or_vocab must be a list of gene names or a GeneVocab "
                "object."
            )
    ...
    def insert_token(self, token: str, index: int) -> None:
        if token in self._stoi:
            raise RuntimeError(f"Token {token!r} already in vocab")
    ...
    def from_file(cls, file_path: Union[Path, str]) -> Self:
        ...
        if file_path.suffix == ".json":
            ...
            with file_path.open("rb") as f:
                obj = pickle.load(f)
                if isinstance(obj, GeneVocab):
                    return obj
                if hasattr(obj, "get_stoi"):
                    return cls.from_dict(obj.get_stoi())
                raise ValueError(
                    f"Pickle at {file_path} contains {type(obj).__name__}, "
                    f"expected GeneVocab or object with get_stoi()"
                )
        raise ValueError(
            f"{file_path} is not a valid file type. "
            "Only .json and .pkl are supported."
        )
...
# Backward-compat alias — pad_batch / tokenize_and_pad_batch annotate `vocab: Vocab`
Vocab = GeneVocab
